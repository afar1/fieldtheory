...
- **[Auth Is Identity, Licenses Are Entitlements](./(apr)-auth-is-identity-licenses-are-entitlements.md)** (v1) - License codes do not simplify a product if they become a second auth or billing system. Keep OTP for identity, Stripe for billing, use the code only to claim paid access, and drive the app from one account-state model.
