...
describe('nextTopNavViewMode', () => {
  ...
  it('falls back to the first tab when starting from a view outside the carousel', () => {
    ...
    expect(nextTopNavViewMode('feedback', 1, true)).toBe('librarian');
    expect(nextTopNavViewMode('sketch', -1, true)).toBe('librarian');
    expect(nextTopNavViewMode('todo', 1, false)).toBe('librarian');
  });
