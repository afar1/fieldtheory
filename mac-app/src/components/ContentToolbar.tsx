...
interface ContentToolbarFolderButtonProps {
  onClick: () => void;
}

export function ContentToolbarFolderButton({ onClick }: ContentToolbarFolderButtonProps) {
      ...
      {(showFolder || showRename || showDelete) && !isEditing && (
        ...
        >
          {showFolder && onShowInFolder && (
            <ContentToolbarFolderButton onClick={onShowInFolder} />
          )}
          {showRename && onRename && (
            <button
              onClick={onRename}
