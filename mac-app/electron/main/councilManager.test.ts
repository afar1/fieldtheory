...
describe('CouncilManager', () => {
  ...
  describe('ensureCommandFile', () => {
    it('writes command file when it does not exist', () => {
      ...
      const mgr = new CouncilManager({
        spawnFn: mockSpawn as any,
        execSyncFn: mockExecSync as any,
        existsSyncFn: mockExists as any,
        mkdirSyncFn: mockMkdirSync as any,
        writeFileSyncFn: mockWriteFileSync as any,
      });

      mgr.ensureCommandFile();

      expect(mockMkdirSync).toHaveBeenCalledWith(
        expect.stringContaining('.fieldtheory/commands'),
        { recursive: true }
      );
      expect(mockWriteFileSync).toHaveBeenCalledWith(
        expect.stringContaining('.fieldtheory/commands/council.md'),
        expect.stringContaining('kickoffs')
      );
